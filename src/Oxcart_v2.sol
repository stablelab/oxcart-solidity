...
/// @custom:oz-upgrades-from Oxcart
contract Oxcart_v2 is Initializable, OwnableUpgradeable {
    ...
    uint lotteryEventsCount;

    // Ticket Power Related
    uint public BASE_TICKETS;
    ...
    function initialize(address initialOwner) public initializer {
        BASE_TICKETS = 1 ether;
        lotteryEventsCount = 0;
    ...
    function manual_override_setLotteryTickets(address[] memory delegates, uint256[] memory _tickets) external onlyOwner {
        ...
        address[] memory participants = topDelegates.values();
    ...
    function getTotalTickets() internal view returns (uint) {
        uint totalTickets = 0;
        for (uint i = 0; i < topDelegates.length(); i++) {
            address delegate = topDelegates.at(i);
    ...
    function drawAndPayoutLottery(uint lotteryId) external onlyOwner returns (address) {
        ...
        for (uint256 i = 0; i < topDelegates.length(); i++) {
            address delegate = topDelegates.at(i);
    ...
    function getLotteryTickets() external view returns (uint[] memory) {
        uint[] memory ticketsList = new uint[](topDelegates.length());
        for (uint256 i = 0; i < topDelegates.length(); i++) {
            address delegate = topDelegates.at(i);
